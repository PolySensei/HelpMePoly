...
/*
void swipeData(std::vector<int>& tour, std::map<int, int>& tabou, std::vector<Bloc>& blocs, int index) {
	if(index < (tour.size() - 1)) {
    		const int range_from  = 7;
    		const int range_to    = 10;
    		std::random_device rand_dev;
		...
		int k = index+1;
		 while(k < tour.size()) {
			if((blocs[tour[k]].w <= blocs[tour[k+1]].w) || (blocs[tour[k]].d <= blocs[tour[k+1]].d)) {
				tabou[tour[k]] = iterationTabou;
				tour.erase(tour.begin() + k);
			} else {
				k++;
...
void tabou(std::vector<Bloc>& blocs)
{
	...
	for (int iteration = 0; iteration < 100; iteration++ ) {
		...
		for (int index = 0; index < blocs.size(); index++ ) {
			...
			if (tempTour.size() == 0) {
				tempTour.push_back(index);
			} else if (((std::find(tempTour.begin(), tempTour.end(), index) == tempTour.end()) && (tempTabou.find(index) == tempTabou.end()))) {
				for (int i = tempTour.size() - 1; i >= 0; i-- ) {
					if((blocs[tempTour[i]].w > blocs[index].w) && (blocs[tempTour[i]].d > blocs[index].d)) {
						...
						std::cout << "tabou size 0 = "<< tempTabou.size()  << std::endl;
						break;
					}
					if(i < 0) {
						tempTour.insert(tempTour.begin(), index);
						swipeData(tempTour, tempTabou, blocs, 0);
      						//std::cout << "tabou size 1 = "<< solutionTabou.size()  << std::endl;
					}
				}
			} //else {
				//continue;
...
std::cout << "solutionTabou size = "<< solutionTabou.size()  << std::endl;
		}
	...
	std::cout << "Ordre des blocs : " << std::endl;
   	for ( int i = 0; i < finalTour.size(); i++ ) {
         	std::cout << blocs[solutionTour[i]].h << " : " << blocs[solutionTour[i]].w  << " : " << blocs[solutionTour[i]].d  << std::endl;
   	}
			...
			if (((std::find(tempTour.begin(), tempTour.end(), index) == tempTour.end()) && (tempTabou.find(index) == tempTabou.end()))) {
				...
				for (; i >= 0; i-- ) {
					if((blocs[tempTour[i]].w > blocs[index].w) && (blocs[tempTour[i]].d > blocs[index].d)) {
						...
						//std::cout << "tabou size = "<< tempTabou.size()  << std::endl;
						break;
					}
					if(i == 0) {
						tempTour.insert(tempTour.begin(), index);

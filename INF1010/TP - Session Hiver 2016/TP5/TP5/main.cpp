...
Date de creation: 11 janvier 2016
...
int main()
{
	...
	Livre *livre1 = new Livre("BD302", "Harry Potter et le prisionier d'Azkaban", 1999, 3, 17, "JK ROWLING", "Science-Fiction"); 
	Livre *livre2 = new Livre("QA204", "Calcul a plusieurs variables partie 2", 2011, 3, 1, "Cay HORTSTMANN", "Informatique");
	Livre *livre3 = new Livre("GA403", "Big C++", 2009, 8, 2, "Cay HORTSTMANN", "Informatique");
	Livre *livre4 = new Livre("AC409", "Le chateau d'Ortrante", 1764, 16, 2, "Cay HORTSTMANN", "Informatique");
	Livre *livre5 = new Livre("QA203", "Calcul a plusieurs variables partie 1", 2011, 3, 1, "Cay HORTSTMANN", "Informatique");
	...
	if (!biblio.ajouterAbonne(tony)) {
		cout << "Echec d'ajouter tony, car il existe deja" << endl;
	}
	if (biblio.retirerAbonne(tony.obtenirMatricule())) {
		cout << "tony est retire" << endl;
	}
	if (biblio.ajouterAbonne(tony)) {
		cout << "tony est ajoute a nouveau" << endl;
	}
	...
	if (!biblio.ajouterObjetEmpruntable(livre4)) {
		cout << "Echec d'ajouter Le chateau d'Ortrante, car il existe deja" << endl;
	}
	if (biblio.retirerObjetEmpruntable(livre4->obtenirCote())) {
		cout << "Le chateau d'Ortrante est retiré" << endl;
	}
	if (biblio.ajouterObjetEmpruntable(livre4)) {
		cout << "Le chateau d'Ortrante est ajouté a nouveau" << endl;
	}
	// Quelques retraits
	biblio -= &hermione;	biblio -= livre3;

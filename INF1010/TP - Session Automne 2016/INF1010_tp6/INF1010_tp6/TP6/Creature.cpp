...
void Creature::attaquer(const Pouvoir & pouvoir, Creature & creature)
{
	...
	if (energie_ >= pouvoir.obtenirEnergieNecessaire() && pouvoirEstDansVector)
	{
		...
		{
            //Si la créature adverse n’a plus de point de vie, une exception de type
            //ExceptionCreatureMorte
            throw ExceptionCreatureMorte("Echec de l'attaque");
		}
	}
	else
	{
        //Si la créature attaquant n’a plus d’énergie, une exception de type
        //ExceptionAttaqueEchouee est lancée
		throw ExceptionAttaqueEchouee("Echec de l'attaque");
	}
}
